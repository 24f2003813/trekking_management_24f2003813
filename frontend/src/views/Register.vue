  ...
  <div class="login-page">
    ...
    <header class="hero-section d-flex align-items-center justify-content-center">
      <div class="card shadow-lg p-4 form-card">
        <h2 class="text-center mb-4" style="color:#8B5E3C;">REGISTER PAGE</h2>
...
export default {
  ...
  methods:{
    async sendRegister() {
        try{
            const rec = await axios.post('http://localhost:5000/api/auth/register',this.form)
            this.$router.push('/login')

        }catch(error){
            console.error("Registration failing",error)
